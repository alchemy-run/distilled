...
export const GetAbuseReportRequest = Schema.Struct({
  ...
  .pipe(T.Http({ method: "GET", path: "/accounts/{account_id}/abuse-reports/{reportParam}" })) as unknown as Schema.Schema<GetAbuseReportRequest>;
...
export interface GetAbuseReportResponse {
  ...
  mitigationSummary: { acceptedUrlCount: number; activeCount: number; externalHostNotified: boolean; inReviewCount: number; pendingCount: number };
  ...
  submitter?: { company?: string; email?: string; name?: string; telephone?: string };
  urls?: string[];
}
...
}).pipe(Schema.encodeKeys({ acceptedUrlCount: "accepted_url_count", activeCount: "active_count", externalHostNotified: "external_host_notified", inReviewCount: "in_review_count", pendingCount: "pending_count" })),
  status: Schema.Literals(["accepted", "in_review"]),
  type: Schema.Literals(["PHISH", "GEN", "THREAT", "DMCA", "EMER", "TM", "REG_WHO", "NCSEI", "NETWORK"]),
...
}).pipe(Schema.encodeKeys({ id: "id", cdate: "cdate", domain: "domain", mitigationSummary: "mitigation_summary", status: "status", type: "type", justification: "justification", originalWork: "original_work", submitter: "submitter", urls: "urls" })) as unknown as Schema.Schema<GetAbuseReportResponse>;
...
export const ListAbuseReportsRequest = Schema.Struct({
  ...
  mitigationStatus: Schema.optional(Schema.Literals(["pending", "active", "in_review", "cancelled", "removed"])).pipe(T.HttpQuery("mitigation_status")),
  ...
  type: Schema.optional(Schema.Literals(["PHISH", "GEN", "THREAT", "DMCA", "EMER", "TM", "REG_WHO", "NCSEI", "NETWORK"])).pipe(T.HttpQuery("type"))
})
  .pipe(T.Http({ method: "GET", path: "/accounts/{account_id}/abuse-reports" })) as unknown as Schema.Schema<ListAbuseReportsRequest>;

export interface ListAbuseReportsResponse {
  reports: ({ id: string; cdate: string; domain: string; mitigationSummary: { acceptedUrlCount: number; activeCount: number; externalHostNotified: boolean; inReviewCount: number; pendingCount: number }; status: "accepted" | "in_review"; type: "PHISH" | "GEN" | "THREAT" | "DMCA" | "EMER" | "TM" | "REG_WHO" | "NCSEI" | "NETWORK"; justification?: string; originalWork?: string; submitter?: { company?: string; email?: string; name?: string; telephone?: string }; urls?: string[] })[] | null;
}

export const ListAbuseReportsResponse = Schema.Struct({
  reports: Schema.Union([Schema.Array(Schema.Struct({
  id: Schema.String,
  cdate: Schema.String,
  domain: Schema.String,
  mitigationSummary: Schema.Struct({
  ...
  urls: Schema.optional(Schema.Array(Schema.String))
}).pipe(Schema.encodeKeys({ id: "id", cdate: "cdate", domain: "domain", mitigationSummary: "mitigation_summary", status: "status", type: "type", justification: "justification", originalWork: "original_work", submitter: "submitter", urls: "urls" }))), Schema.Null])
}) as unknown as Schema.Schema<ListAbuseReportsResponse>;

export type ListAbuseReportsError =
  | DefaultErrors
  | InvalidAccountId;
...
export const CreateAbuseReportRequest = Schema.Struct({
  ...
  .pipe(Schema.encodeKeys({ act: "act", address1: "address1", agentName: "agent_name", agree: "agree", city: "city", country: "country", email: "email", email2: "email2", hostNotification: "host_notification", name: "name", originalWork: "original_work", ownerNotification: "owner_notification", signature: "signature", state: "state", urls: "urls", comments: "comments", company: "company", reportedCountry: "reported_country", reportedUserAgent: "reported_user_agent", tele: "tele", title: "title" }), T.Http({ method: "POST", path: "/accounts/{account_id}/abuse-reports/{reportParam}" })) as unknown as Schema.Schema<CreateAbuseReportRequest>;

export type CreateAbuseReportResponse = string;

export const CreateAbuseReportResponse = Schema.String as unknown as Schema.Schema<CreateAbuseReportResponse>;

export type CreateAbuseReportError =
  | DefaultErrors
  | InvalidRequest;
...
export const ListMitigationsRequest = Schema.Struct({
  ...
  sort: Schema.optional(Schema.Literals(["type,asc", "type,desc", "effective_date,asc", "effective_date,desc", "status,asc", "status,desc", "entity_type,asc", "entity_type,desc"])).pipe(T.HttpQuery("sort")),
  status: Schema.optional(Schema.Literals(["pending", "active", "in_review", "cancelled", "removed"])).pipe(T.HttpQuery("status")),
  type: Schema.optional(Schema.Literals(["legal_block", "phishing_interstitial", "network_block", "rate_limit_cache", "account_suspend", "redirect_video_stream"])).pipe(T.HttpQuery("type"))
})
  .pipe(T.Http({ method: "GET", path: "/accounts/{account_id}/abuse-reports/{reportId}/mitigations" })) as unknown as Schema.Schema<ListMitigationsRequest>;

export type ListMitigationsResponse = ({ mitigations: ({ id: string; effectiveDate: string; entityId: string; entityType: "url_pattern" | "account" | "zone"; status: "pending" | "active" | "in_review" | "cancelled" | "removed"; type: "legal_block" | "phishing_interstitial" | "network_block" | "rate_limit_cache" | "account_suspend" | "redirect_video_stream" })[] })[];

export const ListMitigationsResponse = Schema.Array(Schema.Struct({
  mitigations: Schema.Array(Schema.Struct({
    id: Schema.String,
    effectiveDate: Schema.String,
    entityId: Schema.String,
    entityType: Schema.Literals(["url_pattern", "account", "zone"]),
    status: Schema.Literals(["pending", "active", "in_review", "cancelled", "removed"]),
    type: Schema.Literals(["legal_block", "phishing_interstitial", "network_block", "rate_limit_cache", "account_suspend", "redirect_video_stream"])
  }).pipe(Schema.encodeKeys({ id: "id", effectiveDate: "effective_date", entityId: "entity_id", entityType: "entity_type", status: "status", type: "type" })))
})) as unknown as Schema.Schema<ListMitigationsResponse>;

export type ListMitigationsError =
  | DefaultErrors;
...
export const ReviewMitigationRequest = Schema.Struct({
  ...
  .pipe(T.Http({ method: "POST", path: "/accounts/{account_id}/abuse-reports/{reportId}/mitigations/appeal" })) as unknown as Schema.Schema<ReviewMitigationRequest>;

export type ReviewMitigationResponse = ({ id: string; effectiveDate: string; entityId: string; entityType: "url_pattern" | "account" | "zone"; status: "pending" | "active" | "in_review" | "cancelled" | "removed"; type: "legal_block" | "phishing_interstitial" | "network_block" | "rate_limit_cache" | "account_suspend" | "redirect_video_stream" })[];

export const ReviewMitigationResponse = Schema.Array(Schema.Struct({
  id: Schema.String,
  effectiveDate: Schema.String,
  entityId: Schema.String,
  entityType: Schema.Literals(["url_pattern", "account", "zone"]),
  status: Schema.Literals(["pending", "active", "in_review", "cancelled", "removed"]),
  type: Schema.Literals(["legal_block", "phishing_interstitial", "network_block", "rate_limit_cache", "account_suspend", "redirect_video_stream"])
}).pipe(Schema.encodeKeys({ id: "id", effectiveDate: "effective_date", entityId: "entity_id", entityType: "entity_type", status: "status", type: "type" }))) as unknown as Schema.Schema<ReviewMitigationResponse>;

export type ReviewMitigationError =
  | DefaultErrors;
