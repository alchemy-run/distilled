...
/**
 * Pagination trait for Prisma Postgres APIs.
 *
 * Prisma Postgres uses cursor-based pagination with:
 * - Input: `cursor` (string), `limit`
 * - Output: `pagination.nextCursor` (null if no more pages), items in `data`
 */
export interface PaginatedTrait {
  /** The name of the input member containing the cursor (default: "cursor") */
  inputToken: string;
  /** The path to the output member containing the next cursor (default: "pagination.nextCursor") */
  outputToken: string;
...
/**
 * Default pagination trait for Prisma Postgres APIs.
 */
export const DefaultPaginationTrait: PaginatedTrait = {
  inputToken: "cursor",
  outputToken: "pagination.nextCursor",
  items: "data",
  pageSize: "limit",
};
...
/**
 * Schema for a paginated response from Prisma Postgres APIs.
 */
 ...
 * @param pagination - The pagination trait (defaults to Prisma Postgres standard)
 * @returns A Stream of full page responses
 *
 * @example
 * ```ts
 * const allPages = paginatePages(getV1Projects, {})
 * ```
 */
 ...
 * @example
 * ```ts
 * const allProjects = paginateItems(getV1Projects, {}, { ...DefaultPaginationTrait, items: "data" })
 * ```
 */
