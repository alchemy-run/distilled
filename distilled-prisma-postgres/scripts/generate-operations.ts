...
function toCamelCase(s: string): string {
  // Handle operation IDs like "getV1Projects" - already camelCase
  return s;
}
...
/**
 * Check if a schema type is nullable using OpenAPI 3.1 array type syntax.
 * In 3.1, nullable is expressed as `type: ["string", "null"]` instead of `nullable: true`.
 */
...
/**
 * Get the non-null type from an array type.
 * e.g., ["string", "null"] -> "string"
 */
function getNonNullType(type: string | string[]): string {
  if (Array.isArray(type)) {
...
function openApiTypeToEffectSchema(
  ...
  if (prop.const !== undefined) {
    if (typeof prop.const === "string") {
      return `Schema.Literal("${prop.const}")`;
    }
    return "Schema.Unknown";
  }

  // Determine nullable status (3.0 style or 3.1 style)
  const isNullable = prop.nullable === true || isNullableType(prop.type);

  // Get the actual type (handling 3.1 array syntax)
  const actualType = Array.isArray(prop.type)
    ? getNonNullType(prop.type)
    : prop.type;
  ...
  if (prop.enum && prop.enum.length > 0) {
    ...
    return isNullable ? `Schema.NullOr(${baseSchema})` : baseSchema;
  }
  ...
  switch (actualType) {
    case "string":
      ...
      if (prop["x-sensitive"]) {
        ...
        baseSchema = isNullable ? "SensitiveNullableString" : "SensitiveString";
...
function generateInputSchema(
  ...
  for (const param of pathParams) {
    ...
    const paramType = Array.isArray(schema?.type)
      ? getNonNullType(schema!.type)
      : schema?.type;
    const baseSchema =
...
export type ${outputSchemaName} = typeof ${outputSchemaName}.Type;`,
  ...
  const actualType = Array.isArray(responseSchema.type)
    ? getNonNullType(responseSchema.type)
    : responseSchema.type;
  if (actualType === "array" && responseSchema.items) {
    const itemSchema = openApiTypeToEffectSchema(
...
async function main() {
  ...
  try {
    ...
    console.log("Format command not available, skipping...");
  }
