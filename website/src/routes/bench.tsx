...
const seeded = [bench.runtime?.seed ? "runtime" : "", bench.bundle?.seed ? "bundle" : ""].filter(
  Boolean,
);
      ...
      <Page glow="teal">
        ...
        >
          ...
          <p class="mb-7 max-w-[34em] text-[clamp(1.05rem,1rem+0.35vw,1.2rem)] text-pretty text-fg-2 [&_a]:text-fg">
            Two questions a generated SDK has to answer: how much of your bundle does importing one
            operation drag in, and how much CPU does calling it burn once the network is out of the
            picture. These are the numbers, straight from the harnesses in{" "}
          ...
          <Show when={seeded.length > 0}>
            <p class="m-0 max-w-[60ch] rounded-lg border border-l-[3px] border-[color-mix(in_oklab,var(--accent)_45%,var(--line))] bg-[color-mix(in_oklab,var(--accent)_8%,var(--bg-2))] px-4 py-[0.8rem] text-[0.92rem] text-fg-2 [&_b]:text-fg">
              <b>Interim numbers.</b> The {seeded.join(" and ")} figures below are a hand-entered
              snapshot, not the committed <code>results/latest.json</code>.
            </p>
          </Show>
        </section>
        ...
        <Method id="method" title="Method — how this is measured">
          <li>
            <strong>Bundle.</strong> A tiny worker-shaped entry imports one Distilled operation and
            calls it, bundled with{" "}
            ...
            uses for Cloudflare Workers: <code>bun</code> resolve conditions, minified single ESM
            chunk, PURE annotations on. Size is the chunk; gzip is what the wire sees. Tree-shake
            quality is the number of operations that survive minification out of the service's
            total, with a check that no other service or provider leaked in.
          </li>
          <li>
            <strong>Runtime.</strong> The <code>HttpClient</code> is replaced by one that returns a
            canned response, so what remains is the SDK's own work: schema encode, request build
            (including signing for AWS), response parse, envelope decode, retry wrapper. Numbers are
          ...
          <li>
            ...
            timings move ±30% between runs and machines. Nothing here is compared against a vendor
            SDK yet. Treat it as a shape, not a guarantee.
          </li>
          <li>
            Reproduce: <code>pnpm bench:bundle</code> and <code>pnpm bench:runtime</code> from the
            repo root.
          </li>
        </Method>
      </Page>
