#!/usr/bin/env bun
/**
 * Fetches the Freestyle OpenAPI spec to ../specs/.
 *
 * Freestyle publishes ONE OpenAPI 3.1 document at a stable URL — the same
 * file its TypeScript SDK and CLI are generated from. There is no git repo
 * and no versioned URL, so the mirror simply snapshots it.
 *
 * Usage:
...
async function main() {
  ...
  const response = await fetch(OPENAPI_SPEC_URL, {
    headers: {
      ...
      "user-agent": "distilled.cloud-freestyle-spec-mirror",
    },
  });
