...
export const ItemError: Schema.Schema<ItemError> = Schema.suspend(() => Schema.Struct({
...
export const Item2: Schema.Schema<Item2> = Schema.suspend(() => Schema.Struct({
...
export type GetItemsError = CommonErrors;

/** Gets your own Chrome Web Store item. */
export const getItems: API.OperationMethod<GetItemsRequest, GetItemsResponse, GetItemsError, GCPAuth | HttpClient.HttpClient> = API.make(() => ({
...
export const UpdateItemsRequest = Schema.Struct({
  ...
  T.Http({ method: "PUT", path: "chromewebstore/v1.1/items/{itemId}", hasBody: true }),
  svc,
...
export type UpdateItemsError = CommonErrors;

/** Updates an existing item. */
export const updateItems: API.OperationMethod<UpdateItemsRequest, UpdateItemsResponse, UpdateItemsError, GCPAuth | HttpClient.HttpClient> = API.make(() => ({
...
export const InsertItemsRequest = Schema.Struct({
  publisherEmail: Schema.optional(Schema.String).pipe(T.HttpQuery("publisherEmail")),
}).pipe(
...
export type InsertItemsError = CommonErrors;

/** Inserts a new item. */
export const insertItems: API.OperationMethod<InsertItemsRequest, InsertItemsResponse, InsertItemsError, GCPAuth | HttpClient.HttpClient> = API.make(() => ({
...
export const PublishItemsRequest = Schema.Struct({
  itemId: Schema.String.pipe(T.HttpPath("itemId")),
  deployPercentage: Schema.optional(Schema.Number).pipe(T.HttpQuery("deployPercentage")),
  publishTarget: Schema.optional(Schema.String).pipe(T.HttpQuery("publishTarget")),
  reviewExemption: Schema.optional(Schema.Boolean).pipe(T.HttpQuery("reviewExemption")),
  body: Schema.optional(PublishRequest).pipe(T.HttpBody()),
}).pipe(
  T.Http({ method: "POST", path: "chromewebstore/v1.1/items/{itemId}/publish", hasBody: true }),
  svc,
...
export type PublishItemsError = CommonErrors;

/** Publishes an item. */
export const publishItems: API.OperationMethod<PublishItemsRequest, PublishItemsResponse, PublishItemsError, GCPAuth | HttpClient.HttpClient> = API.make(() => ({
