...
import { type SqliteErrorType } from "../sql/sqlite-error.ts";
import { Sqlite, type SqliteConnection } from "../sql/sqlite.ts";
import { runMigrations } from "./state-store-sqlite-migrations.ts";
import {
  createStateStore,
  StateStore,
  StateStoreError,
...
export const createSqliteStateStoreFromConnection = (conn: SqliteConnection) =>
  Effect.gen(function* () {
    ...
    // Prepare statements
    const stmts = yield* prepareStatements(conn).pipe(
      mapError("prepare statements"),
    );
...
/**
 * Prepare all statements for the state store.
 */
const prepareStatements = (conn: SqliteConnection) =>
  Effect.gen(function* () {
    return {
