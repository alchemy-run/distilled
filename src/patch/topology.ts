 ...
 */
const getOperationType = (opName: string): OperationType => {
...
/**
 * A reference to a value from a previous step's output.
 */
export interface Ref {
  $ref: string; // e.g., "vpc.VpcId"
}

/**
 * A placeholder for a value that needs to be generated.
 */
export interface Placeholder {
  $placeholder: {
    name: string; // The field name
    type: string; // The Smithy type
  };
}

/**
 * An input value - either a literal, a reference, or a placeholder.
 */
export type InputValue = string | number | boolean | Ref | Placeholder;

/**
 * A step in a test sequence - a single API call.
 */
export interface Step {
  /** The operation to call */
  call: string;
  /** Input values, with $ref for references to previous outputs */
  inputs?: Record<string, InputValue>;
  /** Store the output under this name for later reference */
  as?: string;
}

/**
 * A test sequence - a series of API calls to explore error conditions.
 */
...
/**
 * Fields that are effectively required in practice, even if not marked required in Smithy.
 * AWS APIs often require these even when Smithy marks them optional.
 * Map of field name to a default value generator.
 */
const EFFECTIVELY_REQUIRED: Record<string, () => InputValue> = {
  // Network configuration
  CidrBlock: () => ({ $placeholder: { name: "CidrBlock", type: "String" } }),
  // VPC peering
  PeerVpcId: () => ({ $placeholder: { name: "PeerVpcId", type: "VpcId" } }),
  PeerRegion: () => ({
    $placeholder: { name: "PeerRegion", type: "ComponentRegion" },
  }),
  // Transit Gateway attachments
  TransitGatewayId: () => ({
    $placeholder: { name: "TransitGatewayId", type: "TransitGatewayId" },
  }),
  // Instance configuration
  ImageId: () => ({ $placeholder: { name: "ImageId", type: "ImageId" } }),
  // Security groups
  GroupName: () => ({ $placeholder: { name: "GroupName", type: "String" } }),
  // IPAM configuration
  AddressFamily: () => ({
    $placeholder: { name: "AddressFamily", type: "AddressFamily" },
  }),
  Asn: () => ({ $placeholder: { name: "Asn", type: "String" } }),
  // Route server
  PeerAddress: () => ({
    $placeholder: { name: "PeerAddress", type: "String" },
  }),
  BgpOptions: () => ({
    $placeholder: { name: "BgpOptions", type: "BgpOptionsRequest" },
  }),
  // Verified Access
  EndpointType: () => ({
    $placeholder: { name: "EndpointType", type: "VerifiedAccessEndpointType" },
  }),
  AttachmentType: () => ({
    $placeholder: {
      name: "AttachmentType",
  ...
  PolicyRuleNumber: () => ({
    $placeholder: { name: "PolicyRuleNumber", type: "Integer" },
  }),
  MeteredAccount: () => ({
    $placeholder: {
      name: "MeteredAccount",
      type: "TransitGatewayMeteringPayerType",
    },
  }),
  // Traffic mirror
  TrafficDirection: () => ({
    $placeholder: { name: "TrafficDirection", type: "TrafficDirection" },
  }),
  RuleAction: () => ({
    $placeholder: { name: "RuleAction", type: "TrafficMirrorRuleAction" },
  }),
  RuleNumber: () => ({
    $placeholder: { name: "RuleNumber", type: "Integer" },
  }),
  SessionNumber: () => ({
    $placeholder: { name: "SessionNumber", type: "Integer" },
  }),
  // VPN
  Type: () => ({ $placeholder: { name: "Type", type: "String" } }),
  // Destination for routes
  DestinationCidrBlock: () => ({
    $placeholder: { name: "DestinationCidrBlock", type: "String" },
  }),
};

/**
 * Build the inputs for a step, given the operation and available variable bindings.
 * Returns all required inputs, using $ref for references and $placeholder for static values.
 */
const buildStepInputs = (
  graph: DependencyGraph,
  opName: string,
  bindings: Map<string, { resource: string; identifier: string }>,
  ...
  for (const [fieldName, input] of Object.entries(op.inputs)) {
    // Include required fields OR effectively required fields
    const isEffectivelyRequired = fieldName in EFFECTIVELY_REQUIRED;
    if (!input.required && !isEffectivelyRequired) continue;
    hasInputs = true;

    // If this references another resource, try to find a binding
    if (input.references) {
      const refResource = input.references;
      // Find a binding for this resource
      for (const [varName, binding] of bindings) {
        if (binding.resource === refResource) {
          // AWS responses nest the resource: { ResourceName: { Identifier: value } }
          inputs[fieldName] = {
            $ref: `${varName}.${binding.resource}.${binding.identifier}`,
          };
          break;
        }
      }
      // If no binding found, use placeholder
      if (!inputs[fieldName]) {
        inputs[fieldName] = {
          $placeholder: { name: fieldName, type: input.type },
        };
      }
    } else {
      // Static required input - use placeholder
      inputs[fieldName] = {
        $placeholder: { name: fieldName, type: input.type },
      };
    }
...
/**
 * Collect all required ancestors for a resource (transitive dependencies).
 * Returns resources in creation order (roots first).
 */
const collectAncestors = (
  ...
  const res = graph.resources[resourceName];
  if (!res?.operations.create) return [];

  const createOp = graph.operations[res.operations.create];
  if (!createOp) return [];

  const ancestors: string[] = [];

  // Find all required parent references
  for (const input of Object.values(createOp.inputs)) {
    if (!input.required || !input.references) continue;

    const parentName = input.references;
    const parentRes = graph.resources[parentName];

    if (!parentRes?.operations.create || !parentRes?.identifier) continue;

    // Recursively collect ancestors of the parent
    ancestors.push(...collectAncestors(graph, parentName, visited));
...
/**
 * Generate test sequences for dependency violation scenarios.
 * Each sequence: create all ancestors, create parent, create child,
 * delete parent (triggers error), cleanup all in reverse order.
 */
export const generateTestSequences = (
  graph: DependencyGraph,
): TestSequence[] => {
  const sequences: TestSequence[] = [];

  // Find parent-child relationships where child requires parent
  for (const [childName, childRes] of Object.entries(graph.resources)) {
    if (!childRes.operations.create || !childRes.operations.delete) continue;

    const childCreateOp = graph.operations[childRes.operations.create];
    if (!childCreateOp) continue;

    // Look for required references to other resources
    for (const [fieldName, input] of Object.entries(childCreateOp.inputs)) {
      if (!input.required || !input.references) continue;
      ...
      if (!parentRes.operations.create || !parentRes.operations.delete)
        continue;
      if (!parentRes.identifier) continue;

      // Collect all ancestors needed to create the parent
      const ancestors = collectAncestors(graph, parentName);

      // Build the full resource chain: ancestors + parent + child
      const resourceChain = [...ancestors, parentName, childName];

      // Build bindings for all resources
      const allBindings = new Map<
        string,
        { resource: string; identifier: string }
      >();
      for (const resName of resourceChain) {
        const res = graph.resources[resName];
        if (res?.identifier) {
          allBindings.set(resName.toLowerCase(), {
            resource: resName,
      ...
      const parentVar = parentName.toLowerCase();
      const childVar = childName.toLowerCase();

      // Build creation steps for all resources in order
      const createSteps: Step[] = [];
      const bindingsForCreate = new Map<
        string,
        { resource: string; identifier: string }
      >();
      ...
      for (const resName of resourceChain) {
        const res = graph.resources[resName];
        if (!res?.operations.create || !res?.identifier) continue;

        const createInputs = buildStepInputs(
          graph,
          res.operations.create,
          bindingsForCreate,
        );

        createSteps.push({
          call: res.operations.create,
          ...(createInputs && { inputs: createInputs }),
          as: resName.toLowerCase(),
        });

        // Add to bindings for next iterations
        bindingsForCreate.set(resName.toLowerCase(), {
          resource: resName,
          identifier: res.identifier,
        });
      }

      // Build parent delete inputs
      const parentDeleteInputs = buildStepInputs(
        graph,
        parentRes.operations.delete,
        allBindings,
      );

      // Build child delete inputs
      const childDeleteInputs = buildStepInputs(
        graph,
        childRes.operations.delete,
        allBindings,
      );

      // Helper to check if detach step can be satisfied (no placeholder IDs)
      const canSatisfyDetach = (
        inputs: Record<string, InputValue> | undefined,
      ): boolean => {
        if (!inputs) return true;
        // Skip if any ID field is a placeholder (we don't have a real value)
      ...
      // Build cleanup steps for ALL resources in reverse order (child first, ancestors last)
      const cleanupSteps: Step[] = [];
      const reversedChain = [...resourceChain].reverse();

      for (const resName of reversedChain) {
        const res = graph.resources[resName];
        if (!res?.operations.delete) continue;

        // Detach operations first (only if satisfiable)
        for (const detachOp of res.operations.detach) {
          const detachInputs = buildStepInputs(graph, detachOp, allBindings);
          if (canSatisfyDetach(detachInputs)) {
            cleanupSteps.push({
              call: detachOp,
              ...(detachInputs && { inputs: detachInputs }),
            });
          }
        }

        // Delete operation
        const deleteInputs = buildStepInputs(
          graph,
          res.operations.delete,
          allBindings,
        );
        cleanupSteps.push({
          call: res.operations.delete,
          ...(deleteInputs && { inputs: deleteInputs }),
        });
      }

      sequences.push({
        description: `Delete ${parentName} while ${childName} exists`,
        steps: [
          // Create all resources in order (ancestors first, then parent, then child)
          ...createSteps,
          // Try to delete parent (should trigger dependency error)
          {
            call: parentRes.operations.delete,
            ...(parentDeleteInputs && { inputs: parentDeleteInputs }),
          },
...
export const findTestableChains = generateTestSequences;
