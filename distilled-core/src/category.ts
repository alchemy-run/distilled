...
export const withCategory =
  <Category extends string, Categories extends Array<Category>>(
 ...
 */
export const hasCategory = (error: unknown, category: string): boolean => {
  if (
...
/**
 * Check if an error is an HTTP client transport error.
 * These are connection-level failures from Effect's HttpClient (DNS failures,
 * connection refused, TLS errors, etc.) that should be retried.
 */
 ...
 * - HTTP client transport errors (DNS, connection refused, etc.)
 *
 * Vendor SDKs should NOT override this. Instead, tag error classes with
 * `withRetryable()` or the appropriate category at definition time.
 */
export const isTransientError = (error: unknown): boolean => {
  ...
  // Check for HTTP client transport errors
...
export const catchErrors =
  <Categories extends string[], A2, E2, R2>(
...
/**
 * Catch errors with specified categories with type-level narrowing.
 *
 * @example
 * ```ts
 * effect.pipe(
