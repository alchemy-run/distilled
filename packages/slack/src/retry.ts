...
/**
 * Slack retry surface — a veneer over `@distilled.cloud/core/retry`.
 *
 ...
 * Slack calls below it and core's `makeDefault` is the fallback when none is
 * provided. Slack rate limits per method tier and answers 429s with a
 * `Retry-After` header, which the default policy honors with precedence over
 * its own backoff.
 *
 * @example
 * ```ts
 ...
 *
 * myEffect.pipe(Slack.Retry.throttling);
 * ```
 */
...
/** Context tag for configuring retry behavior of Slack API calls. */
export class Retry extends Context.Service<Retry, Policy>()("SlackRetry") {}

/** Provides a custom retry policy to every Slack API call below it. */
export const policy: {
  (
    options: Options,
