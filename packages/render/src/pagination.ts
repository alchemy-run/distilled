 ...
 */
export const paginateRender: Pagination.PaginationStrategy = (operation, input, pagination) => {
  const inputToken = pagination.inputToken;
  if (!inputToken) {
    return Stream.die(new Error("Render cursor pagination requires inputToken"));
  }
  ...
  return Stream.unfold({ cursor: startCursor, done: false } as State, (state) =>
    Effect.gen(function* () {
      ...
      const items = Array.isArray(response) ? response : Pagination.getItems(response, itemsPath);
      const last = items.at(-1) as { cursor?: unknown } | undefined;
      const nextCursor = typeof last?.cursor === "string" ? last.cursor : undefined;
