 ...
 * layer level and have every Gusto API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Gusto from "@distilled.cloud/gusto";
 *
 * myEffect.pipe(Gusto.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Gusto.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Gusto API calls. */
export class Retry extends Context.Service<Retry, Policy>()("GustoRetry") {}

/** Provides a custom retry policy to every Gusto API call below it. */
