 ...
 * layer level and have every Docker API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Docker from "@distilled.cloud/docker";
 *
 * myEffect.pipe(Docker.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Docker.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Docker API calls. */
export class Retry extends Context.Service<Retry, Policy>()("DockerRetry") {}

/** Provides a custom retry policy to every Docker API call below it. */
