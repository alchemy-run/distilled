 ...
 * layer level and have every Hostinger API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Hostinger from "@distilled.cloud/hostinger";
 *
 * myEffect.pipe(Hostinger.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Hostinger.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Hostinger API calls. */
export class Retry extends Context.Service<Retry, Policy>()("HostingerRetry") {}

/** Provides a custom retry policy to every Hostinger API call below it. */
