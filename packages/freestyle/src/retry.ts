/**
 * Freestyle retry surface — a veneer over `@distilled.cloud/core/retry`.
 *
 ...
 * Freestyle calls below it and core's `makeDefault` is the fallback when
 * none is provided.
 *
 * @example
 * ```ts
 ...
 *
 * myEffect.pipe(Freestyle.Retry.transient);
 * ```
 */
...
/** Context tag for configuring retry behavior of Freestyle API calls. */
export class Retry extends Context.Service<Retry, Policy>()("FreestyleRetry") {}

/** Provides a custom retry policy to every Freestyle API call below it. */
export const policy: {
  (
    options: Options,
  ): <A, E, R>(
