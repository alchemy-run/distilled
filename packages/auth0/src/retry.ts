 ...
 * layer level and have every Auth0 API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Auth0 from "@distilled.cloud/auth0";
 *
 * myEffect.pipe(Auth0.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Auth0.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Auth0 API calls. */
export class Retry extends Context.Service<Retry, Policy>()("Auth0Retry") {}

/** Provides a custom retry policy to every Auth0 API call below it. */
