 ...
 * `@distilled.cloud/core/codegen`; this script is Google Workspace's provider spec: the
 * per-service base URL baked into each op's `T.Http` trait, the Google error
...
/** Google Workspace's provider spec for the shared smithy→SDK compiler. */
const makeGoogleWorkspaceSpec = (metadata: { baseUrl?: string }): SdkSpec => ({
  ...
  // the model metadata into the op's Http trait (read by GoogleWorkspaceProtocol).
  ...
  // The standard Google error envelope fields, declared on every generated 4xx
  // error class so catch-site narrowing sees the envelope's gRPC-style
  // `status` and typed `details[]` (populated by GoogleWorkspaceProtocol's tack-on).
  errors: {
    defaultFields: () => [
  ...
  // protocol and traversed by core's default token strategy. Most Google list
  ...
  operationDecl: {
    contextType: "GoogleWorkspaceOpContext",
    commonErrorType: "GoogleWorkspaceOpError",
    commonErrorClasses: ["UnknownGoogleWorkspaceError"],
...
runGeneratorCli({
  description: "Generate the Google Workspace Effect SDK from the Smithy models",
  root: `${import.meta.dir}/..`,
  smithyDir: ".generated-specs/stable",
  outDir: "src/services",
  spec: (model) => makeGoogleWorkspaceSpec(model.metadata ?? {}),
});
