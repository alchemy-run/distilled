 ...
 * on the calling fiber). Auth is an Infisical machine-identity access token
 * sent as `Authorization: Bearer <apiKey>`. Self-hosted instances override
 * the US Cloud default base URL.
 */
...
/** Reads INFISICAL_TOKEN (required) and INFISICAL_API_BASE_URL (optional). */
export const CredentialsFromEnv: Layer.Layer<Credentials> = Layer.succeed(
  Credentials,
  Effect.gen(function* () {
    const apiKey = process.env.INFISICAL_TOKEN;

    if (!apiKey) {
      return yield* new ConfigError({
        message: "INFISICAL_TOKEN environment variable is required",
      });
    }

    return {
      apiKey: Redacted.make(apiKey),
      apiBaseUrl: process.env.INFISICAL_API_BASE_URL ?? DEFAULT_API_BASE_URL,
    };
  }).pipe(Effect.orDie),
);
