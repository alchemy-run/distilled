 ...
 * layer level and have every Inngest API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Inngest from "@distilled.cloud/inngest";
 *
 * myEffect.pipe(Inngest.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Inngest.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Inngest API calls. */
export class Retry extends Context.Service<Retry, Policy>()("InngestRetry") {}

/** Provides a custom retry policy to every Inngest API call below it. */
