 ...
 * layer level and have every Remote API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Remote from "@distilled.cloud/remote";
 *
 * myEffect.pipe(Remote.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Remote.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Remote API calls. */
export class Retry extends Context.Service<Retry, Policy>()("RemoteRetry") {}

/** Provides a custom retry policy to every Remote API call below it. */
