 ...
 * layer level and have every Coolify API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Coolify from "@distilled.cloud/coolify";
 *
 * myEffect.pipe(Coolify.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Coolify.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Coolify API calls. */
export class Retry extends Context.Service<Retry, Policy>()("CoolifyRetry") {}

/** Provides a custom retry policy to every Coolify API call below it. */
