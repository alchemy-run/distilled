 ...
 * GitHub calls below it and core's `makeDefault` is the fallback when none is
 * provided.
 *
 * GitHub answers secondary-rate-limit trips with `403`/`429` plus a
 * `Retry-After` header; the REST protocol stamps that onto the retryable
 * error classes, and the default policy honors it ahead of its own backoff.
 *
 * @example
 * ```ts
 * import * as GitHub from "@distilled.cloud/github";
 *
 * myEffect.pipe(GitHub.Retry.transient);
 * ```
 */
...
/** Context tag for configuring retry behavior of GitHub API calls. */
export class Retry extends Context.Service<Retry, Policy>()("GitHubRetry") {}

/** Provides a custom retry policy to every GitHub API call below it. */
export const policy: {
  (
    options: Options,
