 ...
 * layer level and have every Vanta API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Vanta from "@distilled.cloud/vanta";
 *
 * myEffect.pipe(Vanta.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Vanta.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Vanta API calls. */
export class Retry extends Context.Service<Retry, Policy>()("VantaRetry") {}

/** Provides a custom retry policy to every Vanta API call below it. */
