 ...
 * layer level and have every Argo CD API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Argocd from "@distilled.cloud/argocd";
 *
 * myEffect.pipe(Argocd.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Argocd.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Argo CD API calls. */
export class Retry extends Context.Service<Retry, Policy>()("ArgocdRetry") {}

/** Provides a custom retry policy to every Argo CD API call below it. */
