 ...
 *
 * Model conventions:
 *   • namespace `com.gcp.<name>_<version>`; metadata carries
...
const convertDoc = (doc: DiscoveryDoc): object => {
  const ns = `com.gcp.${ident(doc.name)}_${ident(doc.version)}`;
...
const include = serviceFilter ? (resource: string) => written.has(resource) : undefined;
