...
/**
 * LaunchDarkly retry configuration.
 *
 ...
 * layer level and have every LaunchDarkly API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as LaunchDarkly from "@distilled.cloud/launchdarkly";
 *
 * myEffect.pipe(LaunchDarkly.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(LaunchDarkly.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of LaunchDarkly API calls. */
export class Retry extends Context.Service<Retry, Policy>()("LaunchDarklyRetry") {}

/** Provides a custom retry policy to every LaunchDarkly API call below it. */
