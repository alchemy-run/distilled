/**
 * S2 retry surface — a veneer over `@distilled.cloud/core/retry`.
 *
 ...
 * S2 calls below it and core's `makeDefault` is the fallback when none is
 * provided.
 *
 * @example
 * ```ts
 ...
 *
 * myEffect.pipe(S2.Retry.transient);
 * ```
 */
...
/** Context tag for configuring retry behavior of S2 API calls. */
export class Retry extends Context.Service<Retry, Policy>()("S2Retry") {}

/** Provides a custom retry policy to every S2 API call below it. */
export const policy: {
  (
    options: Options,
  ): <A, E, R>(
