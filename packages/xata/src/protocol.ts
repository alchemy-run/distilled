...
/**
 * XataProtocol — hand-written.
 *
 * Xata's control plane speaks plain bearer-authenticated JSON REST (no
 ...
 *             (default https://api.xata.tech), resolved from the
 ...
 *             HTTP-status classes, then {@link UnknownXataError}.
 */
 ...
 * files annotate operations with `API.OperationMethod<I, O, XataOpError,
 * XataOpContext>` explicitly so the compiler never infers these back out of
 * the schema generics.
 */
export type XataOpError =
...
export const XataProtocol: Layer.Layer<API.Protocol> = makeRestProtocol<Config>({
  ...
  // Xata's error body is `{ message: string, ... }` — the factory's
  // default lenient envelope covers it.
  unknownError: ({ code, message, body }) =>
    new UnknownXataError({
