...
/**
 * Redis Cloud retry configuration.
 *
 ...
 * layer level and have every Redis Cloud API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as RedisCloud from "@distilled.cloud/redis-cloud";
 *
 * myEffect.pipe(RedisCloud.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(RedisCloud.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Redis Cloud API calls. */
export class Retry extends Context.Service<Retry, Policy>()("RedisCloudRetry") {}

/** Provides a custom retry policy to every Redis Cloud API call below it. */
