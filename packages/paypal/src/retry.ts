 ...
 * layer level and have every PayPal API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Paypal from "@distilled.cloud/paypal";
 *
 * myEffect.pipe(Paypal.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Paypal.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of PayPal API calls. */
export class Retry extends Context.Service<Retry, Policy>()("PaypalRetry") {}

/** Provides a custom retry policy to every PayPal API call below it. */
