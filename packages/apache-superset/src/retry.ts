 ...
 * layer level and have every Apache Superset API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as ApacheSuperset from "@distilled.cloud/apache-superset";
 *
 * myEffect.pipe(ApacheSuperset.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(ApacheSuperset.Retry.Retry, customPolicy));
 * ```
 */
