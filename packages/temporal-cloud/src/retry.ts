/**
 * Temporal Cloud retry configuration.
 *
 ...
 * layer level and have every Temporal Cloud API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as TemporalCloud from "@distilled.cloud/temporal-cloud";
 *
 * myEffect.pipe(TemporalCloud.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(TemporalCloud.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Temporal Cloud API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "TemporalCloudRetry",
) {}

/** Provides a custom retry policy to every Temporal Cloud API call below it. */
