 ...
 * `@distilled.cloud/core/codegen`; this script is Temporal Cloud's provider
 * spec. The Cloud Ops API keeps the wire's camelCase member names on the TS
...
/** Temporal Cloud's provider spec for the shared smithy→SDK compiler. */
const temporalCloudSpec: SdkSpec = {
  ...
  // Sensitive strings (the API key token CreateApiKey returns): the schema
  ...
  // names for Temporal Cloud), so no runtime case discrimination is needed.
  union: ({ name, caseTargets, tsRef }) => [
    `export type ${name} = ${caseTargets.map(tsRef).join(" | ") || "unknown"};`,
    `export const ${name} = S.Unknown as any as S.Schema<${name}>;\n`,
  ],

  // One pagination profile: `pageToken` in, `nextPageToken` out, stamped
  // on every Get<Things> collection read in scripts/convert.ts and
  ...
  operationDecl: {
    contextType: "TemporalCloudOpContext",
    commonErrorType: "TemporalCloudOpError",
    commonErrorClasses: ["UnknownTemporalCloudError"],
    protocol: "TemporalCloudProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-temporal-cloud)",
...
runGeneratorCli({
  description: "Generate the Temporal Cloud Effect SDK from the Smithy model",
  root: `${import.meta.dir}/..`,
  // patches/ holds OpenAPI-document patches consumed by scripts/convert.ts;
  // there is no smithy-model patch chain.
  patchesDir: false,
  // `temporal-cloud.ts` → `temporalCloud` in the services barrel.
  barrelExportName: (name) =>
    name.replace(/-([a-z])/g, (_, c: string) => c.toUpperCase()),
  spec: () => temporalCloudSpec,
});
