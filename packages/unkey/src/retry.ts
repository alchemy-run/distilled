 ...
 * layer level and have every Unkey API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Unkey from "@distilled.cloud/unkey";
 *
 * myEffect.pipe(Unkey.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Unkey.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Unkey API calls. */
export class Retry extends Context.Service<Retry, Policy>()("UnkeyRetry") {}

/** Provides a custom retry policy to every Unkey API call below it. */
