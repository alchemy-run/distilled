/**
 * TypeSafe AI retry configuration.
 *
 ...
 * layer level and have every TypeSafe API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as TypesafeAi from "@distilled.cloud/typesafe-ai";
 *
 * myEffect.pipe(TypesafeAi.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(TypesafeAi.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of TypeSafe AI API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "TypesafeAiRetry",
) {}

/** Provides a custom retry policy to every TypeSafe AI API call below it. */
