 ...
 * TypeSafe AI Effect SDK.
 *
 * Input:  .generated-specs/typesafe-ai.json  (written by scripts/convert.ts)
 * Output: src/services/typesafe-ai.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is TypeSafe AI's provider spec.
 * TypeSafe keeps the wire's snake_case member names on the TS surface, so no
 * member renaming or wire dictionaries appear here.
 */
import type { SdkSpec } from "@distilled.cloud/core/codegen/generator";
import { runGeneratorCli } from "@distilled.cloud/core/codegen/cli";

/** `typesafe-ai` → `typesafeAi` (the barrel's export name). */
...
/** TypeSafe AI's provider spec for the shared smithy→SDK compiler. */
const typesafeAiSpec: SdkSpec = {
  ...
  // names for TypeSafe), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "TypesafeAiOpContext",
    commonErrorType: "TypesafeAiOpError",
    commonErrorClasses: ["UnknownTypesafeAiError"],
    protocol: "TypesafeAiProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-typesafe-ai)",
...
runGeneratorCli({
  description: "Generate the TypeSafe AI Effect SDK from the Smithy model",
