 ...
 * layer level and have every STACKIT API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Stackit from "@distilled.cloud/stackit";
 *
 * myEffect.pipe(Stackit.Retry.transient);
 * ```
 */
...
/** Context tag for configuring retry behavior of STACKIT API calls. */
export class Retry extends Context.Service<Retry, Policy>()("StackitRetry") {}

/** Provides a custom retry policy to every STACKIT API call below it. */
