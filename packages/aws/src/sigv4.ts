...
export class InvalidSigningUrl extends Data.TaggedError("AWS::SigV4::InvalidSigningUrl")<{
  readonly url: string;
 ...
 */
export class InvalidSigningHeaders extends Data.TaggedError("AWS::SigV4::InvalidSigningHeaders")<{
  readonly cause: unknown;
}> {}
...
export type SigningError = InvalidSigningUrl | InvalidSigningHeaders | CryptoError;
...
const hmac = (key: string | ArrayBuffer, data: string): Effect.Effect<ArrayBuffer, CryptoError> =>
  Effect.tryPromise({
    try: async () =>
      crypto.subtle.sign(
        "HMAC",
...
const sha256 = (content: SignableBody): Effect.Effect<ArrayBuffer, CryptoError> =>
  Effect.tryPromise({
    try: () => crypto.subtle.digest("SHA-256", toBytes(content)),
    catch: (cause) => new CryptoError({ operation: "sha256", cause }),
  });

const hex = (buffer: ArrayBuffer): string => Encoding.encodeHex(new Uint8Array(buffer));
 ...
 */
export const sign: (options: SignOptions) => Effect.Effect<SignedRequest, SigningError> =
  Effect.fnUntraced(function* (options: SignOptions) {
    const { accessKeyId, service, region, signQuery, allHeaders, body } = options;
    const sessionToken = options.sessionToken ? Redacted.value(options.sessionToken) : undefined;
    ...
    headers.delete("host");
    const datetime = options.datetime ?? new Date().toISOString().replace(/[:-]|\.\d{3}/g, "");
    ...
    const canonicalHeaders = signableHeaders
      .map(
        (header) =>
          header +
          ":" +
          (header === "host" ? url.host : (headers.get(header) ?? "").replace(/\s+/g, " ")),
      )
      .join("\n");
    ...
    if (signQuery) {
      ...
      url.searchParams.set("X-Amz-Credential", accessKeyId + "/" + credentialString);
    ...
    encodedPath = encodeRfc3986(encodeURIComponent(encodedPath).replace(/%2F/g, "/"));
    ...
    const encodedSearch = [...url.searchParams]
      ...
      .map(
        ([k, v]) =>
          [encodeRfc3986(encodeURIComponent(k)), encodeRfc3986(encodeURIComponent(v))] as const,
      )
    ...
    const payloadHash =
      ...
      (service === "s3" && signQuery ? "UNSIGNED-PAYLOAD" : hex(yield* sha256(body || "")));
