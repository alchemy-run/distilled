...
export const generateGraphQLClient = (
  ...
  const rootFn = (
    ...
    if (node) {
      return sig
        ? `  ${fieldName}: (${sig}): Query<${resultTs}> =>\n    rootConnection(${JSON.stringify(kind)}, ${JSON.stringify(fieldName)}, ${node}${argsExpr}${argTypesExpr}),`
        : `  ${fieldName}: (): Query<${resultTs}> => rootConnection(${JSON.stringify(kind)}, ${JSON.stringify(fieldName)}, ${node}),`;
    }
