 ...
 * layer level and have every Okta API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Okta from "@distilled.cloud/okta";
 *
 * myEffect.pipe(Okta.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Okta.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Okta API calls. */
export class Retry extends Context.Service<Retry, Policy>()("OktaRetry") {}

/** Provides a custom retry policy to every Okta API call below it. */
