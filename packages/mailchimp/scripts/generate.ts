...
/**
 * Per-operation error classes that already exist in src/errors.ts. The
 * generator would otherwise emit a second `NotFound` in the service file,
 * which the protocol (mapping by status onto core's) would never construct.
 */
const TRANSACTIONAL_ERROR_CLASSES = [
  "BadRequest",
  "Forbidden",
  "NotFound",
  "PaymentRequired",
...
const transactionalSpec: SdkSpec = {
  ...
  errors: {
    override: ({ name }) =>
      TRANSACTIONAL_ERROR_CLASSES.includes(name) ? [] : undefined,
  },
  header: () =>
  ...
  // Drop the per-operation classes this model never references.
  postProcess: (code) => {
    const body = code.replace(/^import[^;]*;/gm, "");
    return TRANSACTIONAL_ERROR_CLASSES.filter(
      (name) => !new RegExp(`\\b${name}\\b`).test(body),
    ).reduce((next, name) => next.replace(`  ${name},\n`, ""), code);
  },
};
